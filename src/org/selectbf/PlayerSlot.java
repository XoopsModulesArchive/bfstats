...
public class PlayerSlot
{
	...
	public void persists(DatabaseContext dc, PlayerManagementBase pmb) throws SQLException, SelectBfException
	{
		for(Iterator i = assignPeriods.iterator(); i.hasNext();)
		{
			AssignTimePeriod astp = (AssignTimePeriod) i.next();
			
			PreparedStatement pS = dc.prepareStatement("INSERT INTO selectbf_playtimes (player_id, starttime, endtime, time) VALUES (?,?,?,?)");
			
			pS.setInt(1,pmb.lookupDbId(astp.getPlayer()));
			pS.setTimestamp(2,new Timestamp(astp.getStarttime().getTime()));
			pS.setTimestamp(3,new Timestamp(astp.getEndtime().getTime()));
	...
	public Vector getDistinctPlayers()
	{
