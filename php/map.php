...
$cols = SQL_oneRowQuery(
    "select player_id, p.name, score, kills, deaths, attacks, tks, g.id, g.servername,CONCAT(TIME_FORMAT(r.starttime,'%H:%i:%S '),DATE_FORMAT(r.starttime,'%d|%m|%Y')) time from selectbf_playerstats ps, selectbf_players p, selectbf_rounds r, selectbf_games g where ps.player_id = p.id and ps.round_id = r.id and r.game_id = g.id and g.map = '$map' order by score DESC LIMIT 0,1"
);

$tmpl->setVar("best_player", $cols["name"]);
$tmpl->setVar("best_playerdetaillink", "player.php?id=" . $cols["player_id"]);
$tmpl->setVar("best_score", $cols["score"]);
$tmpl->setVar("best_kills", $cols["kills"]);
$tmpl->setVar("best_deaths", $cols["deaths"]);
$tmpl->setVar("best_attacks", $cols["attacks"]);
$tmpl->setVar("best_tks", $cols["tks"]);
$tmpl->setVar("best_gamedetaillink", "game.php?id=" . $cols["id"]);
$tmpl->setVar("best_gamename", $cols["servername"]);
$tmpl->setVar("best_time", $cols["time"]);
...
$tmpl->setLoop("drives", getMapDrives($map));
