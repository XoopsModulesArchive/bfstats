...
// +----------------------------------------------------------------------+
//
// $Id: vlibTemplate.php,v 1.18 2003/10/02 11:16:53 releasedj Exp $

// check to avoid multiple including of class
if (!defined('vlibTemplateClassLoaded')) {
    define('vlibTemplateClassLoaded', 1);

    require_once __DIR__ . '/vlibTemplate/error.php';
    require_once __DIR__ . '/vlibIni.php';
    ...
    class vlibTemplate
    {
        ...
        function setDbLoop($loopname, $result, $db_type = 'MYSQL')
        {
            ...
            switch ($db_type) {
                ...
                case 'INFORMIX':
                    ...
                    while (false !== ($r = ifx_fetch_row($result, 'NEXT'))) {
                        $loop_arr[] = $r;
                    }
                    break;
                ...
                case 'INTERBASE':
                    ...
                    while (false !== ($r = ibase_fetch_row($result))) {
                        $loop_arr[] = $r;
                    }
                    break;
                ...
                case 'INGRES':
                    ...
                    while (false !== ($r = ingres_fetch_array(INGRES_ASSOC, $result))) {
                        $loop_arr[] = $r;
                    }
                    break;
                ...
                case 'MSSQL':
                    ...
                    while (false !== ($r = mssql_fetch_array($result))) {
                        $loop_arr[] = $r;
                    }
                    break;
                ...
                case 'MSQL':
                    ...
                    while (false !== ($r = msql_fetch_array($result, MSQL_ASSOC))) {
                        $loop_arr[] = $r;
                    }
                    break;
                ...
                case 'SYBASE':
                    ...
                    while (false !== ($r = sybase_fetch_array($result))) {
                        $loop_arr[] = $r;
                    }
                    break;
            }
            ...
            if (is_array($arr) && !empty($arr)) {
                ...
                for ($i = 0; $i < count($arr); $i++) {
                    ...
                    if ($this->OPTIONS['LOOP_CONTEXT_VARS']) {
                        if ($i == 0) {
                            $arr[$i]['__FIRST__'] = true;
                        }
                        if (($i + 1) == count($arr)) {
                            $arr[$i]['__LAST__'] = true;
                        }
                        if ($i != 0 && (($i + 1) < count($arr))) {
                            $arr[$i]['__INNER__'] = true;
                        }
                        if (is_int(($i + 1) / 2)) {
                            $arr[$i]['__EVEN__'] = true;
                        }
        ...
        function _parseIf($varname, $value = null, $op = null, $namespace = null)
        {
            ...
            if (count($this->_namespace) == 0 || $namespace == 'global') {
                return '$this->_vars[\'' . $varname . '\']' . $comp_str;
            }
    ...
    require_once __DIR__ . '/vlibTemplate/debug.php';
    require_once __DIR__ . '/vlibTemplate/cache.php';
} // << end if(!defined())..
?>
