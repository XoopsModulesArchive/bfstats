...
class vlibTemplateDebug extends vlibTemplate
{
     ...
     */
    var $_debug = 1;
    var $_debugIncludedfiles = [];
    var $_debugTemplatevars  = [];
    var $_debugalltags = [];
    var $_debugtags    = [];
    var $_debugwarnings           = [];
    var $_debugwarningmsgscontrol = [];
    var $_debugwarningmsgs        = [];
    ...
    function doDebug()
    {
        ...
        $params = ['vlibTemplate Root' => $this->VLIBTEMPLATE_ROOT];
        ...
        if (!is_array($this->_debugTemplatevars['include'])) {
            $this->_debugTemplatevars['include'] = [];
        }
        ...
        if (!is_array($this->_debugTemplatevars['elseif'])) {
            $this->_debugTemplatevars['elseif'] = [];
        }
        ...
        if (!is_array($this->_debugTemplatevars['unless'])) {
            $this->_debugTemplatevars['unless'] = [];
        }
    ...
    function arrangeTags($arr)
    {
        ...
        for ($i = 1; $i < 4; $i++) {
            if (empty($arr['paramname' . $i]) && empty($arr['paramval' . $i])) {
                break;
            }
    ...
    function doDebugParse()
    {
        ...
        foreach ($this->_debugalltags as $v) {
            ...
            if (eregi("^([a-z0-9_]+\.)*([a-z0-9_]+[_a-z0-9]*)$", $var, $matches) && !ereg('include|comment', $tag)) {
                $var = $matches[2];
            }
                    ...
                    if ($lasttag[0] == 'unless') {
                        array_push(
                            ...
                            [
                                ...
                                'detail'   => "The following end loop tag was found as a closing tag to a tmpl_unless:" . $entire_tag . ", whilst this 'may' not cause an error, it is incorrect vlibTemplate syntax (this maybe because of previous errors).",
                                'location' => 'Line: ' . $line . ', in file: ' . $file,
                            ]
                        );
                        array_pop($this->_debugtags);
                        ...
                        array_push(
                            ...
                            [
                                ...
                                'detail'   => "The following end loop tag was found as a closing tag to a tmpl_if:" . $entire_tag . ", whilst this 'may' not cause an error, it is incorrect vlibTemplate syntax (this maybe because of previous errors).",
                                'location' => 'Line: ' . $line . ', in file: ' . $file,
                            ]
                        );
                    ...
                    if ($lasttag[0] == 'unless') {
                        array_push(
                            ...
                            [
                                ...
                                'detail'   => "The following end if tag was found as a closing tag to a tmpl_unless:" . $entire_tag . ", whilst this may not cause an error, it is incorrect vlibTemplate syntax (this maybe because of previous errors).",
                                'location' => 'Line: ' . $line . ', in file: ' . $file,
                            ]
                        );
                        array_pop($this->_debugtags);
                    ...
                    if ($lasttag[0] == 'if') {
                        array_push(
                            ...
                            [
                                ...
                                'detail'   => "The following end unless tag was found as a closing tag to a tmpl_if:" . $entire_tag . ", whilst this may not cause an error, it is incorrect vlibTemplate syntax (this maybe because of previous errors).",
                                'location' => 'Line: ' . $line . ', in file: ' . $file,
                            ]
                        );
                        array_pop($this->_debugtags);
    ...
    {
        $this->_debugIncludedfiles      = [];
        $this->_debugTemplatevars       = [];
        $this->_debugtags               = [];
        $this->_debugwarnings           = [];
        $this->_debugwarningmsgscontrol = [];
        $this->_debugwarningmsgs        = [];
    }
