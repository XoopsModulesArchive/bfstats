...
// +----------------------------------------------------------------------+
//
// $Id: vlibMimeMail.php,v 1.3 2002/12/11 00:21:56 releasedj Exp $
...
if (!defined('vlibMimeMailClassLoaded')) {
    ...
    require_once __DIR__ . '/vlibCommon/common.php';

    // get error reporting functionality.
    require_once __DIR__ . '/vlibMimeMail/error.php';
    ...
    class vlibMimeMail
    {
        ...
        var $all_emails           = []; // if above is true, will contain all addies
        ...
        function cc($ccEmail, $ccName = null)
        {
            ...
            if ($this->checkAddress && !$this->validateEmail($ccEmail)) {
                vlibMimeMailError::raiseError('VM_ERROR_BADEMAIL', FATAL, 'Cc: ' . $ccEmail);
            }
            if ($this->apply_windows_bugfix) {
                array_push($this->all_emails, $ccEmail);
            }
        ...
        function bcc($bccEmail, $bccName = null)
        {
            ...
            if ($this->checkAddress && !$this->validateEmail($bccEmail)) {
                vlibMimeMailError::raiseError('VM_ERROR_BADEMAIL', FATAL, 'Bcc: ' . $bccEmail);
            }
            if ($this->apply_windows_bugfix) {
                array_push($this->all_emails, $bccEmail);
            }
        ...
        function from($fromEmail, $fromName = null)
        {
            ...
            if ($this->checkAddress && !$this->validateEmail($fromEmail)) {
        ...
        function replyTo($replytoEmail, $replytoName = null)
        {
            ...
            if ($this->checkAddress && !$this->validateEmail($replytoEmail)) {
                vlibMimeMailError::raiseError('VM_ERROR_BADEMAIL', FATAL, 'Reply-To: ' . $replytoEmail);
            }
        ...
        function htmlBody($htmlbody, $charset = '')
        {
            ...
            if ($charset != '') {
                ...
                if ($this->htmlcharset != 'us-ascii') {
                    $this->htmlctencoding = '8bit';
                }
            }
        }
        ...
        {
            if ($mimetype === null) {
                $mimetype = vlibCommon::getMimeType($filename);
            }
        ...
        function _buildMail()
        {
            ...
            if (!empty($this->sendcc)) {
                $this->xheaders[$cc_header_name] = implode(',', $this->sendcc);
            }
            if (!empty($this->sendbcc)) {
                $this->xheaders['Bcc'] = implode(',', $this->sendbcc);
            }
        ...
        function _build_attachments()
        {
            ...
            for ($i = 0; $i < count($this->attachments); $i++) {
                ...
                if (preg_match('/^[a-zA-Z]+:\/\//', $filename)) { // figure out if local or remote
                    ...
                    if (!empty($upart['user'])) {
                        $newfilename .= $upart['user'] . ':' . $upart['pass'] . '@';
                    }
                    $newfilename .= $upart['host'];
                    if (!empty($upart['port'])) {
                        $newfilename .= ':' . $upart['port'];
                    }
                    $newfilename .= '/';
                    if (!empty($upart['path'])) {
                        ...
                        for ($i = 0; $i < count($newpath); $i++) {
                            $newpath[$i] = rawurlencode($newpath[$i]);
                        }
                        $newfilename .= implode('/', $newpath);
                    }
                    if (!empty($upart['query'])) {
                        $newfilename .= '?' . urlencode($upart['query']);
                    }
                    if (!empty($upart['fragment'])) {
                        $newfilename .= ':' . $upart['fragment'];
                    }
