...
if (!defined('vlibDateClassLoaded')) {
    ...
    require_once __DIR__ . '/vlibDate/error.php';
    ...
    class vlibDate
    {
        ...
        var $accepted_langs = ['en', 'fr', 'de', 'es', 'pt', 'nl', 'it', 'no', 'sv', 'da', 'fi', 'ro', 'ar', 'ru'];
        /*-----------------------------------------------------------------------------\
        |                           public functions                                   |
        \-----------------------------------------------------------------------------*/
        ...
        {
            [$day, $month, $year] = array_values($this->_breakTimestamp($timestamp));
        ...
        function setLang($lang = 'en')
        {
            if (in_array($lang, $this->accepted_langs)) {
                require __DIR__ . '/vlibDate/langrefs_' . $lang . '.php';
                $this->days     = $days;
        ...
        function isLeapYear($year = "")
        {
            if (strlen($year) > 4) { // timestamp has bee parsed
                [$day, $month, $year] = array_values($this->_breakTimestamp($year));
        ...
        function daysInMonth($month = "", $year = "")
        {
            if (strlen($month) > 2) { // timestamp has bee parsed
                [$day, $month, $year] = array_values($this->_breakTimestamp($month));
            } else {
                if (empty($year)) {
                    $year = $this->_dateNow("%Y");
                }
        ...
        function _calcSubInterval($number, $interval)
        {
            ...
            switch ($interval) {
                ...
                case 'YEAR':
                    if ($number > 0) {
                        for ($i = 0; $i < $number; $i++) {
                            if (($month < 3 && $this->isLeapYear(($year - 1))) or ($month > 2 && $this->isLeapYear($year))) {
                                $this->_interval += 366;
                            } else {
                                $this->_interval += 365;
                            }
        ...
        function _calcAddInterval($number, $interval)
        {
            ...
            switch ($interval) {
                ...
                case 'YEAR':
                    if ($number > 0) {
                        for ($i = 0; $i < $number; $i++) {
                            if (($month < 3 && $this->isLeapYear($year)) or ($month > 2 && $this->isLeapYear(($year + 1)))) {
                                $this->_interval += 366;
                            } else {
                                $this->_interval += 365;
                            }
