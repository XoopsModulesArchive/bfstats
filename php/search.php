...
if ($search == "players") {
    ...
    $servername = $GLOBALS['xoopsDB']->escape($servername);
    $day        = $_REQUEST["day"];
    ...
    if ($year == "nothing" || $month == "nothing" || $day == "nothing") {
        ...
        $res = SQL_query(
            "select id, servername,modid mod,map,game_mode, CONCAT(TIME_FORMAT(starttime,'%H:%i:%S '),DATE_FORMAT(starttime,'%d|%m|%Y')) time from selectbf_games WHERE servername LIKE '%$servername%' AND starttime BETWEEN '$year-$month-$day 00:00:00' and '$year-$month-$day 23:59:59' and modid='$mod' ORDER BY starttime ASC"
        );
    }
    while (false !== ($cols = SQL_fetchArray($res))) {
...
if ($DEBUG > 0) {
    echo("<b><i>gathering Search results</i></b> took $totaltime secs<br>");
}
